...
label_match() {
  if echo "$PR_FILES" | grep -E "$2"; then
    echo "+[\"$1\"]"
  fi
}
...
LABELS=$LABELS$(label_match mod:cuda ^cuda/)
LABELS=$LABELS$(label_match mod:hip ^hip/)
...
LABELS=$LABELS$(label_match type:reordering /reordering/)
